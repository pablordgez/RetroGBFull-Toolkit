...
void load_animation(uint8_t x, uint8_t y){
    ...
    THIS_ANIMATION_STATE->sprite_slot = register_space(&sprite_space_manager, THIS_ANIMATION->width * THIS_ANIMATION->height / 16);
    if(THIS_ANIMATION->already_loaded == 0){
        THIS_ANIMATION->already_loaded++;
        THIS_ANIMATION->start_tile = register_space(&sprite_tile_manager, THIS_ANIMATION->width * THIS_ANIMATION->height / 16);
        set_sprite_data(THIS_ANIMATION->start_tile, THIS_ANIMATION->width * THIS_ANIMATION->height * THIS_ANIMATION->number_of_frames / 16, entry->data);

...
void play_music_step(void) NONBANKED{
    ...
    const Pattern* p1 = current_song->ch1_seq[current_pattern_index];
    const Pattern* p2 = current_song->ch2_seq[current_pattern_index];
    const Pattern* p4 = current_song->ch4_seq[current_pattern_index];
    ...
    if (p1 != NULL) {
        ...
        if (s1.note_index != NOTE_REST) {
            Instrument inst = current_song->instruments[active_inst_ch1];
    ...
    if (p2 != NULL) {
        ...
        if (s2.note_index != NOTE_REST) {
            Instrument inst = current_song->instruments[active_inst_ch2];
    ...
    if (p4 != NULL) {
        ...
        if (s4.note_index != NOTE_REST) { 
            Instrument inst = current_song->instruments[active_inst_ch4];
            NR41_REG = inst.reg1;
            NR42_REG = inst.reg2;
            NR43_REG = inst.reg3; 
            NR44_REG = 0x80;      

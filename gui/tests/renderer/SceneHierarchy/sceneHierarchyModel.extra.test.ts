...
describe('sceneHierarchyModel edge cases', () => {
  it('clones documents, builds defaults, and formats scene coordinates', () => {
    ...
    expect(buildDefaultSceneNode('folder', 'Group', () => 'f')).toMatchObject({ id: 'f', type: 'folder' })
    expect(buildDefaultSceneNode('collision', 'Hit', () => 'c')).toMatchObject({ id: 'c', isBlocking: true })
    expect(buildDefaultSceneNode('actor', 'Hero', () => 'a')).toMatchObject({ id: 'a', physicsMode: 'balanced' })
  ...
  it('finds, updates, collects, translates, and clears scene subtrees', () => {
    const nodes = [folder([actor({ id: 'parent', children: [collision(), actor({ id: 'child' })] })])]
    ...
    expect(findSceneNodePathById(nodes, 'child')?.map((node) => node.id)).toEqual(['folder', 'parent', 'child'])
    ...
    expect(updateSceneNodeById(nodes, 'child', (node) => ({ ...node, name: 'Child 3' }))[0].children[0].children[1]).toMatchObject({
      name: 'Child 3'
    })
    expect(updateSceneNodeIfPresent(nodes, 'missing', vi.fn())).toMatchObject({ found: false })
    expect(updateSceneNodeIfPresent(nodes, 'child', (node) => ({ ...node, name: 'Child 2' }))).toMatchObject({
      found: true
    })
    ...
    expect(mapSceneNodes(nodes, (node) => (node.id === 'child' ? { ...node, name: 'Mapped' } : node))[0].children[0].children[1]).toMatchObject({
      name: 'Mapped'
    })
    ...
    expect(translateSceneNodeSubtreeSpatial(collision({ x: 1, y: 2 }), 3, 4)).toMatchObject({ x: 4, y: 6 })
    expect(clearFollowCameraInSceneNodeSubtree(folder([actor({ followCamera: true })]))).toMatchObject({
      children: [expect.objectContaining({ followCamera: false })]
    })
    expect(clearFollowCameraInSceneNodeSubtree(actor({ followCamera: true, children: [collision()] }))).toMatchObject({
      followCamera: false,
    ...
    expect(buildUniqueSceneNodeName([actor({ name: 'Actor' }), actor({ id: 'keep', name: 'Actor 2' })], 'Actor', 'keep')).toBe('Actor 2')
  ...
  it('validates scene insertion and paste targets', () => {
    ...
    const nestedActor = actor({ id: 'nested', children: [collision({ id: 'c1' }), collision({ id: 'c2' })] })
    ...
    expect(canInsertSceneNodeAtParent(nodes, 'parent', collision({ id: 'new-collider' }))).toBe(false)
    ...
    expect(isValidScenePasteTarget(nodes, { operation: 'copy', node: actor({ id: 'copy' }), sourceNodeId: null }, null)).toBe(true)
    expect(isValidScenePasteTarget(nodes, { operation: 'cut', node: actor(), sourceNodeId: null }, null)).toBe(false)
    expect(isValidScenePasteTarget(nodes, { operation: 'cut', node: parent, sourceNodeId: 'parent' }, 'parent')).toBe(false)
    expect(isValidScenePasteTarget(nodes, { operation: 'cut', node: parent, sourceNodeId: 'parent' }, 'missing')).toBe(false)
  })
})
...
describe('sceneDocumentEditorCommands edge cases', () => {
  it('clamps nodes, edits actors/collisions, and ignores no-op changes', () => {
    ...
    expect(buildCollisionUpdateChange(nodes, 'hit', { x: 2015, y: 2025, isBlocking: false })?.nodes[0].children[0]).toMatchObject({
      x: 15,
      y: 25,
      isBlocking: false
    })
  ...
  it('builds clipboard, paste, rename, tag, resource, script, and callback changes', () => {
    const nodes = [folder([actor({ id: 'actor', name: 'Actor', followCamera: true, tags: ['old'], children: [collision({ id: 'hit' })] })])]

    expect(buildSceneNodeDeletionChange(nodes, { operation: 'cut', node: nodes[0], sourceNodeId: 'actor' }, 'folder')?.clipboard).toBeNull()
    expect(buildSceneNodeDeletionChange(nodes, null, 'missing')).toBeNull()
    expect(buildSceneClipboardChange(nodes, 'actor', 'copy')?.clipboard?.node).toMatchObject({ followCamera: false })
    expect(buildSceneClipboardChange(nodes, 'missing', 'copy')).toBeNull()
    expect(buildScenePasteChange(nodes, { operation: 'cut', node: nodes[0], sourceNodeId: 'missing' }, null)).toBeNull()
    expect(buildScenePasteChange(nodes, { operation: 'copy', node: actor({ name: 'Actor' }), sourceNodeId: null }, 'folder')?.selectedNodeId).toBeTruthy()

    expect(buildSceneRenameChange(nodes, { nodeId: 'actor', originalName: 'Actor', draftName: ' Actor ' })).toBeNull()
    expect(buildSceneRenameChange(nodes, { nodeId: 'missing', originalName: 'Actor', draftName: 'New' })).toBeNull()
    expect(buildSceneRenameChange(nodes, { nodeId: 'actor', originalName: 'Actor', draftName: 'Folder' })?.nodes[0].children[0]).toMatchObject({ name: 'Folder' })

    expect(buildSceneNodeTagsChange(nodes, 'folder', ['a'])).toBeNull()
    expect(buildSceneNodeTagsChange(nodes, 'actor', [])?.nodes[0].children[0]).toMatchObject({ tags: undefined })
    ...
    expect(buildActorResourcePathChange(nodes, 'actor', 'actor.rgbactor.json')?.[0].children[0]).toMatchObject({
      resourcePath: 'actor.rgbactor.json'
    })
    expect(buildActorScriptPropertyChange(nodes, 'actor', 'hp', 1)?.selectedNodeId).toBe('actor')
    expect(buildActorScriptPropertyChange(nodes, 'actor', 'hp', 1)?.nodes[0].children[0]).toMatchObject({
    ...
    expect(buildCollisionCallbacksChange(nodes, 'hit', [callback])?.nodes[0].children[0].children[0]).toMatchObject({
      callbacks: [callback]
    })
  ...
  it('loads actor resources with placement and rejects invalid parents', () => {
    ...
    const loaded = buildLoadedActorChange(nodes, null, actor({ id: 'source', name: 'Actor', x: 1, y: 2, followCamera: true }), {
      x: 10,
      y: 20
    }, 'Actors/Hero.rgbactor.json')
    expect(loaded.nodes[1]).toMatchObject({ name: 'Actor 2', x: 10, y: 20, followCamera: false, resourcePath: 'Actors/Hero.rgbactor.json' })

    expect(() =>
      buildLoadedActorChange(nodes, 'parent', actor({ children: [collision({ id: 'c1' }), collision({ id: 'c2' })] }))
